...
unsafe fn init_versions() -> Result<(), SignatureError> {
    VERSION = PROC.scan_sig(VERSION_SIG, &VERSION_OFFSETS)? as *mut c_char;
    Ok(())
}
...
pub unsafe fn init_versions_or_show_error() -> bool {
    if let Err(e) = init_versions() {
        let text = format!(
            "The essential version signatures weren't found. Please wait for an update.\n\n{:?}",
            e
        );
        let title = "Outdated!";
...
pub unsafe fn get_version() -> Option<String> {
    ...
    Some(
        CStr::from_bytes_until_nul(&version)
            .unwrap()
            .to_string_lossy()
            .to_string(),
    )
